...
def test_habitat_r2r_ppo_uninavid_config_selects_gae_actor_critic():
    ...
    assert cfg.algorithm.rollout_epoch == 2
    assert cfg.env.train.total_num_envs == 32
    assert cfg.env.train.max_episode_steps == 128
    assert cfg.env.train.max_gt_action_length == 85
    assert dict(cfg.env.train.bucket_step_range_map) == {"all": [0, 85]}
    assert list(cfg.env.train.bucket_max_steps) == [128]
    assert list(cfg.env.train.curriculum_stages_map.stage_all) == [2]
    assert cfg.env.eval.max_gt_action_length == 85
