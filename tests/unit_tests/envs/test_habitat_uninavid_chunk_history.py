...
def test_habitat_step_trace_logging_appends_required_record(tmp_path):
    class EnvStub:
        def __init__(self):
            self.actions = []
        ...
        def step(self, actions):
            ...
            return (
                ...
                np.array([0.0], dtype=np.float32),
                np.array([False]),
                [{"distance_to_goal": 1.5, "trajectory_Length": 0.25}],
            )

        def get_current_episode_metadata(self):
            return {"episode_id": ["24"]}

    env = object.__new__(HabitatEnv)
    env.cfg = SimpleNamespace(
        model_type="uninavid",
        video_cfg=SimpleNamespace(save_video=False),
        habitat_step_trace_dir=str(tmp_path),
    )
    env.env = EnvStub()
    env.num_envs = 1
    env._elapsed_steps = np.array([0], dtype=np.int32)
    ...
    env.prev_step_reward = np.array([0.0], dtype=np.float32)
    env.current_raw_obs = None
    env.initial_distance_to_goal = np.array([1.5], dtype=np.float32)
    env._uninavid_early_stop_last_distance_to_goal = np.array(
        [1.5], dtype=np.float32
    )
    env._uninavid_early_stop_rotation_counts = np.array([0], dtype=np.int32)
    ...
    env._save_metrics = lambda infos, masks: None
    env._overlay_first_done_episode_metrics = lambda infos: None
    env._wrap_obs = lambda raw_obs, info_lists=None: {"wrist_images": torch.zeros(1)}

    env.step(np.array(["move_forward"]))

    trace_path = tmp_path / "episode_24.jsonl"
    ...
    assert records == [
        {
            ...
            "proposed_action": "move_forward",
            "rotation_count": 0,
            "trajectory_Length": 0.25,
        }
    ]
