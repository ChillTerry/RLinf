...
class PatchWeightSyncer(WeightSyncer):
    def __init__(
        ...
        self.init_sync_bucket_size = int(init_sync_bucket_size)
        if self.init_sync_bucket_size <= 0:
            raise ValueError("Patch init sync bucket size must be positive")
        ...
        for key, value in state_dict.items():
            for prefix in self.init_sync_prefixes:
                if key == prefix or key.startswith(f"{prefix}."):
                    ...
                    break
        unmatched = [key for key, matched in matched_prefixes.items() if not matched]
        if unmatched:
            raise ValueError(
