...
def _find_best_bottleneck_swap(
    ...
    *,
    ...
    for source_group_idx, source_group_slots in enumerate(
        ...
        for target_slot_idx in range(len(process_slot_loads[source_process_idx])):
            ...
            for target_group_idx, target_group_slot_ids in enumerate(
                ...
                if (
                    source_load_after >= current_peak
                    or target_load_after >= current_peak
                ):
                    continue
...
def _assign_bottleneck_local_search(
    ...
    *,
    ...
    while True:
        ...
        if (
            not repair_succeeds
            or _peak_for_process_slot_loads(candidate_loads) >= current_peak
        ):
            break
