...
class UniNaVIDMetaForCausalLM(ABC):
    ...
    def process_tensor(
        ...
        assert result_tensor.shape[1] == sum(result_list), (
                ...
                while image_token_indices.numel() > 0:
                    ...
                    if getattr(self.config, "tune_mm_mlp_adapter", False) and getattr(
                        ...
                        if (
                            ...
                            assert final_token_length_lst[cur_image_idx] is not None
                            video_index = 0
                            for ii_index, ii in enumerate(
                                final_token_length_lst[cur_image_idx]
                            ):
                                cur_new_input_embeds.append(
                                    cur_image_features[video_index : video_index + ii]
                                )
                                if (
                                    ii_index
                                    == len(final_token_length_lst[cur_image_idx]) - 1
                                ):
                                    break
